...
void handle_upload(int client_socket, struct sockaddr_in *server_addr, socklen_t addr_len, const char *filename) {
    ...
    while (1) {
        ...
        int encrypted_size = encrypt_data(plaintext, bytes_read, ciphertext);
        memcpy(data.data, ciphertext, encrypted_size);
...
void handle_download(int client_socket, struct sockaddr_in *server_addr, socklen_t addr_len, const char *filename) {
...
void handle_delete(int client_socket, struct sockaddr_in *server_addr, socklen_t addr_len, const char *filename) {
    request_packet req;
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(command, "upload") == 0 && filename) {
    handle_upload(client_socket, &server_addr, addr_len, filename);
  } else if (strcmp(command, "download") == 0 && filename) {
    handle_download(client_socket, &server_addr, addr_len, filename);
  } else if (strcmp(command, "delete") == 0 && filename) {
    handle_delete(client_socket, &server_addr, addr_len, filename);
  } else {
    display_usage();
  }
